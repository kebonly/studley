...
if __name__ == "__main__":
    ...
    res = set()
    for filename in os.listdir(PATH_OBSIDIAN):
        modify_time = datetime.datetime.fromtimestamp(os.path.getmtime(f"{PATH_OBSIDIAN}/{filename}"))
        if (modify_time >= daystart) and os.path.isfile(f"{PATH_OBSIDIAN}/{filename}") and (filename not in OMIT):
            if filename[-3:] == ".md":
                res.add(filename[:-3])
            else:
                res.add(filename)
